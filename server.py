...
class DividendPayoutRatioReport(BaseModel):
    ...
    Dividend_Payout_Ratio_report: str = Field(
        description="Detailed analysis of Dividend Payout Ratio"
    )


class StockPriceReport(BaseModel):
    stock_price_trending: str = Field(
        description="Trending of Stock Price(Upward, Downward, Hold)"
    )
...
class AnalysisReport(BaseModel):
    ...
    stock_new: StockNews = Field(
        description="Relating News of Stock. You should contain title, summmary, sentiment, url"
    )
...
class Tools:
    ...
    def get_FFO(symbol):
        ...
        try:
            nonOperatingIncome = list(
                stock.income_stmt.loc[["Other Non Operating Income Expenses"]]
                .dropna(axis=1)
                .to_dict()
                .values()
            )
            ...
            netIncome = list(
                stock.income_stmt.loc[["Net Income"]].dropna(axis=1).to_dict().values()
            )
...
async def ROE_analyst(context: str, symbol: str):
    ...
    prompt = ChatPromptTemplate.from_messages(
        ...
        """,
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not. Give me recommendation in terms of ROE rate.",
            ),
        ]
    )
    ...
    result = await chain.ainvoke(
        {"context": context, "ROE_info": ROE_info, "ROE": ROE, "symbol": symbol}
    )
...
async def PER_analyst(context: str, symbol: str):
    ...
    prompt = ChatPromptTemplate.from_messages(
        ...
        """,
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not. Give me recommendation in terms of PER rate.",
            ),
        ]
    )
    ...
    result = await chain.ainvoke(
        {"context": context, "PER_info": PER_info, "PER": PER, "symbol": symbol}
    )
...
async def FFO_analyst(context: str, symbol: str):
    ...
    prompt = ChatPromptTemplate.from_messages(
        ...
        """,
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not. Give me recommendation in terms of FFO rate.",
            ),
        ]
    )
    ...
    result = await chain.ainvoke(
        {"context": context, "FFO_info": FFO_info, "FFO": FFO, "symbol": symbol}
    )
...
async def dividend_payout_ratio_analyst(context: str, symbol: str):
    ...
    prompt = ChatPromptTemplate.from_messages(
        ...
        """,
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not. Give me recommendation in terms of dividend payout ratio.",
            ),
        ]
    )
...
async def stock_price_analyst(symbol):
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not. Give me recommendation in terms of stock price",
            ),
        ]
    )
...
async def hedge_fund_manager(company):
    ...
    prompt = ChatPromptTemplate.from_messages(
        ...
        """,
            ...
            (
                "human",
                "Give me recommendation whether {symbol} is good to invest or not.",
            ),
        ]
    )
    ...
    result = chain.invoke(
        {
            ...
            "PER": information[1],
            "FFO": information[2],
            "payout_ratio": information[3],
            "news_analysis": news,
            "price_analysis": information[5],
            "format_instructions": parser.get_format_instructions(),
        }
    )
